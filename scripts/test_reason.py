...
推理模型会先在 <think>...</think> 中展示思考过程，
再在 <answer>...</answer> 中给出最终答案。
...
用法：
    ...
    # 手动对话
    python scripts/test_reason.py --mode chat

    # 自定义参数
    python scripts/test_reason.py --device cpu --max_new_tokens 500
"""
...
def main():
    parser = argparse.ArgumentParser(description="测试推理蒸馏模型的思维链能力")
    parser.add_argument('--weight_dir', default='out', type=str, help="权重目录")
    parser.add_argument('--weight_name', default='reason', type=str, help="权重名称前缀")
    ...
    # ---------- 3. 对话 ----------
    print("\n" + "=" * 60)
    print("推理模型思维链测试（观察 <think> 和 <answer> 标签）")
    print("=" * 60)
    ...
    for prompt in prompts:
        ...
        # 不跳过特殊 token，保留 <think>/<answer> 标签便于观察
