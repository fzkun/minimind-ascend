"""
测试 GRPO（Group Relative Policy Optimization）模型的推理能力。

GRPO 模型默认以 reasoning 模式训练，输出包含 <think>/<answer> 标签。

用法：
    python scripts/test_grpo.py
    python scripts/test_grpo.py --mode chat
    python scripts/test_grpo.py --device cpu --max_new_tokens 500
"""
...
def main():
    parser = argparse.ArgumentParser(description="测试 GRPO 模型的推理能力")
    parser.add_argument('--weight_dir', default='out', type=str, help="权重目录")
    parser.add_argument('--weight_name', default='grpo', type=str, help="权重名称前缀")
    ...
    # ---------- 3. 对话 ----------
    print("\n" + "=" * 60)
    print("GRPO 模型推理测试（观察 <think> 和 <answer> 标签）")
    print("=" * 60)
