"""
测试 PPO（近端策略优化）模型的推理能力。

PPO 模型默认以 reasoning 模式训练，输出包含 <think>/<answer> 标签。

用法：
    python scripts/test_ppo.py
    python scripts/test_ppo.py --mode chat
    python scripts/test_ppo.py --device cpu --max_new_tokens 500
"""
...
def main():
    parser = argparse.ArgumentParser(description="测试 PPO 模型的推理能力")
    parser.add_argument('--weight_dir', default='out', type=str, help="权重目录")
    parser.add_argument('--weight_name', default='ppo_actor', type=str, help="权重名称前缀")
    ...
    # ---------- 3. 对话 ----------
    print("\n" + "=" * 60)
    print("PPO 模型推理测试（观察 <think> 和 <answer> 标签）")
    print("=" * 60)
