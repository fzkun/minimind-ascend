"""
测试 DPO（直接偏好优化）模型的对话能力。

DPO 模型在 SFT 基础上通过 chosen/rejected 对比学习了人类偏好。

用法：
    python scripts/test_dpo.py
    python scripts/test_dpo.py --mode chat
    python scripts/test_dpo.py --device cpu --max_new_tokens 200
"""
...
def main():
    parser = argparse.ArgumentParser(description="测试 DPO 模型的对话能力")
    parser.add_argument('--weight_dir', default='out', type=str, help="权重目录")
    parser.add_argument('--weight_name', default='dpo', type=str, help="权重名称前缀")
